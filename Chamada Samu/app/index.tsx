...
import { Stack, useRouter } from 'expo-router'; // Importar Stack aqui é importante
...
export default function LoginScreen() {
  ...
  const handleAuth = () => {
    ...
    if (isLogin) {
      ...
      if (result.success) {
        Alert.alert("Sucesso", "Conta criada!", [{ text: "OK", onPress: () => irParaApp() }]);
      } else Alert.alert("Erro", result.message);
    }
  };
  ...
  return (
    <KeyboardAvoidingView behavior={Platform.OS === 'ios' ? 'padding' : 'height'} style={styles.container}>
      {/* Remove qualquer barra de navegação desta tela específica */}
      ...
      <View style={styles.formContainer}>
        ...
        <View style={styles.buttonWrapper}>
          <Button title={isLogin ? "ENTRAR" : "CADASTRAR"} color="#d32f2f" onPress={handleAuth} />
        </View>
        <TouchableOpacity onPress={() => setIsLogin(!isLogin)} style={styles.switchButton}>
          <Text style={styles.switchText}>{isLogin ? "Criar conta" : "Fazer Login"}</Text>
        </TouchableOpacity>
      </View>
